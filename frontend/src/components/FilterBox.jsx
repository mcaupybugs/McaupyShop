...
const FilterBox = ({ projectDetails, changeProjectBasedOnFilter }) => {
  ...
  const [selectedProjects, setSelectedProjects] = useState(projectDetails);
  ...
  const clearTags = () => {
    var allInputTags = document.getElementsByClassName("sort-by-tags");

    // Convert HTMLCollection to an array and reset all checkboxes
    Array.from(allInputTags).forEach((element) => (element.checked = false));
    setSelectedTags([]);
  };
